...
# "训练集"的对比
plt.subplot(2, 1, 1) 
plt.plot(np.arange(len(result_train)), y_trainRON, "bo-", label="True value 摩阻系数lambda - train") # 真实值                   # b：blue，o：circle 蓝色圆点
plt.plot(np.arange(len(result_train)), resultRON_train, "ro-", label="Predict value 摩阻系数lambda - train") # 预测值           # r：red，o：circle 红色圆点
plt.title(f"train_SVR---R^2:{score_train}") # 图名 
plt.legend(loc="best") # 位置 

# "测试集"的对比
plt.subplot(2, 1, 2)
plt.plot(np.arange(len(result)), y_testRON, "bo-", label="True value 摩阻系数lambda - test")
plt.plot(np.arange(len(result)), resultRON, "ro-", label="Predict value 摩阻系数lambda - test")
